...
/* Definitions for counter_semaphore */
osSemaphoreId_t counter_semaphoreHandle;
const osSemaphoreAttr_t counter_semaphore_attributes = {
  .name = "counter_semaphore"
};
/* USER CODE BEGIN PV */
...
void counter_task(void *argument);
...
int main(void)
{
  ...
  /* creation of counter_semaphore */
  counter_semaphoreHandle = osSemaphoreNew(1, 0, &counter_semaphore_attributes);
  ...
  CounterTaskHandle = osThreadNew(counter_task, NULL, &CounterTask_attributes);
...
{
    osSemaphoreRelease(counter_semaphoreHandle);
...
/* USER CODE BEGIN Header_counter_task */
/**
 * @brief  Function implementing the CounterTask thread.
 * @param  argument: Not used
 * @retval None
 */
/* USER CODE END Header_counter_task */
void counter_task(void *argument)
{
  /* USER CODE BEGIN 5 */
    uint8_t counter = 0;
    ...
    for (;;)
    {
        ...
        osSemaphoreAcquire(counter_semaphoreHandle,  4000);
        counter++;
